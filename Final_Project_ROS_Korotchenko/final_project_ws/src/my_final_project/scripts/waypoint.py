...
WAYPOINTS = [[2.67,2.95],[7.2,3.13],[6.99,-4.94],[3.36,-5.0],[3.06,-6.76],[-1.21,-7.1],[-1.15,-3.41],[0.649,-2.81],[0,0],[0.649,-2.81],[-1.15,-3.41],[-1.21,-7.1],[3.06,-6.76],[3.36,-5.0],[6.99,-4.94],[7.2,3.13],[2.67,2.95],[0,0]]
...
class turtlebot_move():
    def __init__(self):
        ...
        self.vel_pub = rospy.Publisher('part2_cmr/cmd_vel', Twist, queue_size=10)
        self.vel = Twist()
    ...
    def move_to_point(self, x, y):
        ...
        while not rospy.is_shutdown():
            ...
            if abs(linear) > 0.2:
                linear = linear/abs(linear)*0.2*5.5

            angular = self.pid_theta.update(self.theta)
            if abs(angular) > 0.2:
                angular = angular/abs(angular)*0.2*7.5
